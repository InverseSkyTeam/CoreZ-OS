...
int main(int argc, char** argv) {
    ...
    int intact = 1;
    for (int i = 0; i < 0x3000 / 4; i++) {
        if (wp[i] != (uint32_t)(i * 0x13 + 5)) { intact = 0; break; }
    }
    ...
    CHECK(mmap2(0, 0, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0) == MAP_FAILED,
          "mmap2 len 0 rejects");

    void* h = mmap2((void*)1, 0x1000, PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    CHECK(h != MAP_FAILED && ((uint32_t)h & 0xfff) == 0, "mmap2 unaligned hint ignored");
    if (h != MAP_FAILED) {
        munmap(h, 0x1000);
    }
