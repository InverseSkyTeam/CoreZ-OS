...
uint8_t gfx_gray(int level) {
    if (level < 0) level = 0; if (level > 23) level = 23;
...
void gfx_fill(struct gfx_canvas* c, int x, int y, int w, int h, uint8_t color) {
    ...
    size_t last;
    if (__builtin_mul_overflow((size_t)v.y + (size_t)v.h - 1, (size_t)c->pitch, &last)) return;
    if (__builtin_add_overflow(last, (size_t)v.x + (size_t)v.w - 1, &last)) return;
    if (last >= mapped) return;
    for (int row = 0; row < v.h; row++) {
        uint8_t* p = c->pixels + (v.y + row) * c->pitch + v.x;
        for (int i = 0; i < v.w; i++) p[i] = color;
    }
}
...
void gfx_char(struct gfx_canvas* c, int x, int y, char ch, uint8_t fg, int bg) {
    ...
    for (int row = 0; row < 16; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            ...
            if (bits & (0x80 >> col)) c->pixels[py * c->pitch + px] = fg;
            else if (bg >= 0)         c->pixels[py * c->pitch + px] = (uint8_t)bg;
        }
    }
}

void gfx_text(struct gfx_canvas* c, int x, int y, const char* s, uint8_t fg, int bg) {
    while (*s) {
...
void gfx_char_scaled(struct gfx_canvas* c, int x, int y, char ch, uint8_t fg, int bg, int scale) {
    if (scale < 1) scale = 1;
    const uint8_t* font = FONT_BASE + ((uint8_t)ch) * 16;
    for (int row = 0; row < 16; row++) {
        uint8_t bits = font[row];
...
void gfx_text_scaled(struct gfx_canvas* c, int x, int y, const char* s, uint8_t fg, int bg, int scale) {
    while (*s) {
...
void gfx_blit(struct gfx_canvas* dst, int dx, int dy,
    ...
    if (src->w <= 0 || src->h <= 0 || src->pitch <= 0 ||
        dst->w <= 0 || dst->h <= 0 || dst->pitch <= 0) return;
    if (src->pitch < src->w || dst->pitch < dst->w) return;
    if (!dst->pixels || !src->pixels) return;

    int ok;
    size_t src_size = gfx_canvas_mapped_bytes(src, &ok); if (!ok || src_size == 0) return;
    size_t dst_size = gfx_canvas_mapped_bytes(dst, &ok); if (!ok || dst_size == 0) return;

    size_t src_last, dst_last;
    if (__builtin_mul_overflow((size_t)sy + (size_t)h - 1, (size_t)src->pitch, &src_last)) return;
    if (__builtin_add_overflow(src_last, (size_t)sx + (size_t)w - 1, &src_last)) return;
    if (src_last >= src_size) return;

    if (__builtin_mul_overflow((size_t)dy + (size_t)h - 1, (size_t)dst->pitch, &dst_last)) return;
    if (__builtin_add_overflow(dst_last, (size_t)dx + (size_t)w - 1, &dst_last)) return;
    if (dst_last >= dst_size) return;
