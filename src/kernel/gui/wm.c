...
struct wl_surface* wm_focused_surface(void) {
    ...
    if (ws->focus < 0 || ws->focus >= ws->n) return 0;
...
static void arrange(void) {
    struct workspace* ws = &g_ws[g_cur_ws];
    struct gfx_rect area = {0, COMP_BAR_H, comp_screen_w(), comp_screen_h() - COMP_BAR_H};
    ...
    if (tn > 0) {
        ...
        for (int i = 0; i < tn; i++) tiled[i] = ws->order[i];
    ...
    for (int i = tn; i < ws->n; i++) comp_damage_surface(ws->order[i]);
    bar_invalidate();
}
...
void wm_manage(struct wl_surface* s) {
    struct workspace* ws = &g_ws[g_cur_ws];
    if (ws->n >= WL_MAX_SURFACES) return;
    s->ws = g_cur_ws;
    s->floating = 0;
    
    for (int i = ws->n; i > ws->tcount; i--) ws->order[i] = ws->order[i - 1];
...
void wm_unmanage(struct wl_surface* s) {
    ...
    if (idx < 0) return;
    comp_damage_surface(s);
    if (g_drag == s) g_drag = 0;
    for (int i = idx; i < ws->n - 1; i++) ws->order[i] = ws->order[i + 1];
    ws->n--;
    if (idx < ws->tcount) ws->tcount--;
    if (ws->n == 0) ws->focus = -1;
    else if (ws->focus >= ws->n) ws->focus = ws->n - 1;
    else if (idx < ws->focus) ws->focus--;
    comp_log("wm: window closed");
    if (s->ws == g_cur_ws) arrange();
}
...
static void focus_index(int idx) {
    ...
    if (idx < 0) idx = ws->n - 1;
    if (idx >= ws->n) idx = 0;
    if (idx == ws->focus) return;
    struct wl_surface* old = wm_focused_surface();
    if (old) comp_damage_surface(old);
    ws->focus = idx;
...
static void ws_switch(int target) {
    if (target == g_cur_ws || target < 0 || target >= WL_MAX_WS) return;
    
    struct workspace* old = &g_ws[g_cur_ws];
    for (int i = 0; i < old->n; i++) comp_damage_surface(old->order[i]);
    g_cur_ws = target;
...
static void move_focused_to(int target) {
    ...
    if (dst->n >= WL_MAX_SURFACES) return;

    int idx = ws_index_of(cur, s);
    int was_float = idx >= cur->tcount;
    comp_damage_surface(s);
    for (int i = idx; i < cur->n - 1; i++) cur->order[i] = cur->order[i + 1];
    cur->n--;
    if (!was_float) cur->tcount--;
    if (cur->n == 0) cur->focus = -1;
    else if (cur->focus >= cur->n) cur->focus = cur->n - 1;

    s->ws = target;
    if (was_float) {
        dst->order[dst->n++] = s;
    } else {
        for (int i = dst->n; i > dst->tcount; i--) dst->order[i] = dst->order[i - 1];
...
static void toggle_float(void) {
    ...
    if (idx < ws->tcount) {
        ...
        for (int i = idx; i > ws->tcount; i--) ws->order[i] = ws->order[i - 1];
        ws->order[ws->tcount] = s;
        ws->tcount++;
        ws->focus = ws->tcount - 1;
        s->floating = 0;
...
static void swap_focused(int dir) {
    struct workspace* ws = &g_ws[g_cur_ws];
    int idx = ws->focus;
    if (idx < 0 || idx >= ws->tcount) return;   
    int other = idx + dir;
    if (other < 0) other = ws->tcount - 1;
    if (other >= ws->tcount) other = 0;
    if (other == idx) return;
...
void wm_handle_key(uint8_t scancode, int pressed, uint8_t mods) {
    ...
    if (!(mods & MOD_ALT)) {
        ...
        if (f) comp_send_key(f, scancode, pressed, mods);
        return;
    }
    ...
    if (scancode >= SC_1 && scancode < SC_1 + WL_MAX_WS) {
        ...
        if (shift) move_focused_to(target);
        else ws_switch(target);
        return;
    }
    ...
    case SC_ENTER:
        ...
        break;
    case SC_J:
        if (shift) swap_focused(1);
        else focus_index(g_ws[g_cur_ws].focus + 1);
        break;
    case SC_K:
        if (shift) swap_focused(-1);
        else focus_index(g_ws[g_cur_ws].focus - 1);
    ...
    case SC_H:
        ...
        if (g_ws[g_cur_ws].mfact < 100) g_ws[g_cur_ws].mfact = 100;
        arrange();
        break;
    case SC_L:
        g_ws[g_cur_ws].mfact += 50;
        if (g_ws[g_cur_ws].mfact > 900) g_ws[g_cur_ws].mfact = 900;
        arrange();
        break;
    case SC_SPACE:
...
static struct wl_surface* hit_test(int x, int y, int* on_title, int* on_close) {
    ...
    for (int i = ws->n - 1; i >= 0; i--) {
        ...
        if (on_title) *on_title = (y < fy + COMP_TITLE_H);
        if (on_close) *on_close = (y < fy + COMP_TITLE_H) && (x >= fx + fw - 18);
        return s;
    }
    return 0;
}
...
void wm_handle_button(int x, int y, uint8_t buttons, uint8_t edge) {
    if (edge & 1) {
        if (buttons & 1) {
            ...
            if (!s) { g_drag = 0; return; }
            struct workspace* ws = &g_ws[g_cur_ws];
            int idx = ws_index_of(ws, s);
            if (idx >= 0) focus_index(idx);
            if (on_close) {
                comp_send_close(s);
                return;
            }
...
void wm_handle_motion(int x, int y) {
    ...
    s->x = x - g_drag_dx;
    s->y = y - g_drag_dy;
    if (s->x < -s->w + 32) s->x = -s->w + 32;
    if (s->x > comp_screen_w() - 32) s->x = comp_screen_w() - 32;
    if (s->y < COMP_BAR_H + COMP_TITLE_H) s->y = COMP_BAR_H + COMP_TITLE_H;
    if (s->y > comp_screen_h() - 24) s->y = comp_screen_h() - 24;
    comp_damage_surface(s);
}
...
int wm_collect_visible(struct wl_surface** out, int max) {
    ...
    for (int i = 0; i < n; i++) out[i] = ws->order[i];
    return n;
}
