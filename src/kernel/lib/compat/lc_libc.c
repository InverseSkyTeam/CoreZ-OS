...
long lc_write(int fd, const void *buf, uint32_t n) {
    return (long)__lc_syscall6(LC_WRITE, (uint32_t)fd, (uint32_t)buf, n, 0, 0,
                               0);
}

long lc_read(int fd, void *buf, uint32_t n) {
    return (long)__lc_syscall6(LC_READ, (uint32_t)fd, (uint32_t)buf, n, 0, 0,
                               0);
}
...
long lc_open(const char *path, int flags) {
    return (long)__lc_syscall6(LC_OPEN, (uint32_t)path, (uint32_t)flags, 0, 0,
                               0, 0);
}
...
long lc_set_thread_area(void *base) {
    return (long)__lc_syscall6(LC_SET_THREAD_AREA, (uint32_t)base, 0, 0, 0, 0,
                               0);
}

long lc_writev(int fd, const struct lc_iovec *iov, int iovcnt) {
    return (long)__lc_syscall6(LC_WRITEV, (uint32_t)fd, (uint32_t)iov,
                               (uint32_t)iovcnt, 0, 0, 0);
}
