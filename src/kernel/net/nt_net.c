...
static void http_handler(struct mg_connection* c, int ev, void* ev_data) {
    ...
    if (ev == MG_EV_OPEN) printf("[N] open\n");
    if (ev == MG_EV_READ) printf("[N] read\n");
...
static void net_thread(void* arg) {
    ...
    if (mg_http_listen(&mgr, "http://0.0.0.0:8765", http_handler, NULL) == NULL) {
