...
static int rtl8139_find_dev(uint8_t* bus, uint8_t* dev) {
    for (uint16_t d = 0; d < 32; d++) {
        ...
        if ((id & 0xFFFF) == RTL8139_VENDOR && ((id >> 16) & 0xFFFF) == RTL8139_DEVICE) {
            *bus = 0;
            *dev = (uint8_t)d;
            return 1;
        }
...
static size_t rtl8139_tx(const void* buf, size_t len, struct mg_tcpip_if* ifp) {
    ...
    for (uint32_t i = 0; i < 100000; i++) {
        if (inl(ioaddr + REG_TSD0 + tx_slot * 4) & (1 << 13)) break;
    }
    tx_slot = (tx_slot + 1) & 3;
    return len;
}
