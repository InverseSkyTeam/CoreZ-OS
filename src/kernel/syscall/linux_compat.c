// 参考: Wine syscall 翻译(gitlab.winehq.org/wine), Linux set_thread_area(glibc user_desc)
...
static int32_t compat_write(int32_t fd, const void* buf, uint32_t count) {
    if (fd < 0) return -1;
    if (is_pipe(fd)) return (int32_t)pipe_write(fd, buf, count);
...
static int32_t compat_read(int32_t fd, void* buf, uint32_t count) {
    ...
    if (is_pipe(fd)) return (int32_t)pipe_read(fd, buf, count);
...
static uint32_t compat_brk_alloc(struct task_struct* cur, uint32_t len) {
    ...
    if (sys_brk(want) != want) return (uint32_t)-1;
    return curbrk;
}

static int32_t compat_set_thread_area(struct Registers* r, struct task_struct* cur, uint32_t base) {
    if (base == 0) return -1;
    cur->tls_base = base;
...
static int32_t sys_compat_writev(int32_t fd, struct lc_iovec* iov, int32_t iovcnt) {
...
uint32_t linux_compat_handler(struct Registers* r) {
    ...
    case LC_MMAP: {
        kprintf("[compat] mmap(0x%x,%u,prot=0x%x,flags=0x%x,fd=%u,off=%u)  <- 6 arg\n",
                a, b, c, d, e, f);
        if (a != 0) {
            ret = a;
        } else {
    ...
    case LC_WRITEV: {
        int32_t n = sys_compat_writev((int32_t)a, (struct lc_iovec*)b, (int32_t)c);
        lc_seterrno(cur, n < 0 ? -n : 0);
        ret = n < 0 ? (uint32_t)-1 : (uint32_t)n;
        break;
    }
