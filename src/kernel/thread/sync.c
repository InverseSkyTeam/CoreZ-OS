...
void lock_acquire(struct lock* plock) {
    if (plock->holder != current_task || plock->holder == 0) {
        ...
        if (plock->holder_repeat_nr != 0) {
            asm_cli();
            kprintf("\n[LOCK ACQ BUG] plock=0x%x old_holder=0x%x cur=0x%x rn=%d sval=%d caller=0x%x\n",
...
void lock_release(struct lock* plock) {
    if (plock->holder != current_task) {
        asm_cli();
        kprintf("\n[LOCK REL BUG] plock=0x%x holder=0x%x cur=0x%x rn=%d caller=0x%x\n",
                (uint32_t)plock, (uint32_t)plock->holder, (uint32_t)current_task,
